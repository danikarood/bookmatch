...
if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
    $upload_dir = '../../assets/images/user-avatars/';
    ...
    $new_file_name = 'avatar_' . time() . '_' . uniqid() . '.' . $file_extension;
    $upload_path = $upload_dir . $new_file_name;
    
    // Move uploaded file
    if (move_uploaded_file($file_tmp, $upload_path)) {
        $avatar_path = '../assets/images/user-avatars/' . $new_file_name;
    } else {
...
$_SESSION['user_profile'] = [
    'name' => $name,
    'bio' => $bio,
    'avatar' => !empty($avatar_path) ? $avatar_path : $_SESSION['user_profile']['avatar'] ?? ''
];
